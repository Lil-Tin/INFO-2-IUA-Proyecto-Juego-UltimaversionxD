...
void vibora::constructor() {
    ...
    tCabeza.loadFromFile("IMAGENES/Cabeza.png");
    sCabeza.setTexture(tCabeza);
...
void vibora::dibujar(RenderWindow* pantalla , int direccion) {
    ...
    if (aux1->sig==NULL) {
        sCabeza.setPosition(aux1->posicionX, aux1->posicionY);
        pantalla->draw(sCabeza);
...
bool vibora::comio(Sprite sComida) {
    ...
    if(sCabeza.getGlobalBounds().intersects(sComida.getGlobalBounds())){
        return  true;
    }
...
bool vibora::detectarColisionesFondo() {
    return frente->posicionX < 17 || frente->posicionX > 621 - 10 || frente->posicionY < 64 - 10 ||
           frente->posicionY > 461 - 10;
    }
